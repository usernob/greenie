<main class="my-2">
    <div class="container">
        ...
        <form action="<?= BASE_URL ?>/cart/request" method="post" class="flex flex-col gap-2" id="form-cart">
            <div class="p-4 bg-slate-100 shadow-md rounded-md">
                <div class="grid grid-cols-[2.25rem_minmax(500px,_2fr)_1fr_1fr_1fr] gap-y-4">
                    ...
                    foreach ($data["db"]["barang"] as $barang) : $i++; ?>
                        <input type="checkbox" class="w-5 h-5 checked:bg-main mr-auto ml-0 my-auto" id="checkbox-produk" value="<?= $barang["id_cart"]; ?>" data-target="<?= BASE_URL ?>/cart/delete/<?= $barang["id_cart"] ?>">
                        <div class="flex items-center gap-8">
                            ...
                            <?php endif ?>
                            <a href="<?= BASE_URL ?>/home/detail/<?= $barang["id_barang"] ?>" class="text-lg break-all flex-1 hover:text-main"><?= $barang["nama_barang"] ?></a>
                        </div>
                        <h2 class="text-lg text-center m-auto text-slate-900" data-price="<?= $barang["harga_barang"] ?>" id="price-before">
                    ...
                    <?php endforeach ?>
                    <?php if ($i <= 0) : ?>
                        <div class="col-span-full py-14">
                            <h2 class="text-center text-lg text-slate-500">Tidak ada item dalam keranjang</h2>
                        </div>
                    <?php endif ?>
                </div>
            </div>
            <div class="p-4 bg-slate-100 shadow-md rounded-md flex justify-between items-center">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <button type="submit" class="bg-main hover:bg-green-600 px-6 py-2 rounded-md text-slate-100" id="submit-cart" data-target="<?= BASE_URL ?>">Continue</button>
                </div>
            </div>
        </form>
    </div>
